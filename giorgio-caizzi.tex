\documentclass[letterpaper,11pt]{article}

%-----------------------------------------------
% import from CV.sty with the necessary packages
% document definition and custom commands
\usepackage{CV}

%-----------------------------------------------
% CV starts here
\begin{document}

%----------HEADING----------
\begin{center}
  \textbf{\Huge \bfseries Giorgio Caizzi} \\
  \vspace{3pt}
  \small +39 340 8247713 | \href{mailto:giocaizzi@gmail.com}{\underline{giocaizzi@gmail.com}}
  | Milan, Italy\\
  \vspace{3pt}
  \faLinkedinSquare{} : \href{https://linkedin.com/in/giorgio-caizzi/}{\underline{giorgio-caizzi}}
  \faGithubSquare{} : \href{https://www.github.com/giocaizzi/}{\underline{giocaizzi}}\\
\end{center}
\vspace{-30pt}

%-----------EXPERIENCE-----------
\section{Experience}
% start list
\begin{sectionElementsList}
  % aecom
  \experienceElement
  {Environmental Engineer}{Feb. 2021}
  {AECOM URS Italia S.p.A.}{Milan, Italy}
  {
    Technical consultancy on remediation procedures in industrial sites across Italy, in compliance with D.Lgs.152/06.
  }{

    {
        Developing custom solutions for effective collection, processing, analysis and restitution of environmental data.

      },
    {
        Project management: proposals, bidding, planning, budgeting and billing. Regular meetings with clients.
      },
    {
        Design and implementation of investigation and remediation activities for contaminated water and soils.
      },
    {
        Field supervision of technical activities carried out by subcontractors.
      },
    {
        Relations with national authorities and local authorities  (ARPA, Ministry of Environment, Municipalities).
      }
  }
  % %wiseair
  \experienceElement
  {External Collaborator}{Mar. 2020 -- Dec. 2020}
  {Wiseair S.r.l.}{Milan, Italy \& Lisbon, Portugal}
  {
    Development of a GIS-based framework for spatial interpolation of PM2.5 concentration
    data, using real-time data from a dense network of measuring stations and taking into account the contribution of
    urban morphology in the dispersion of pollutants.
  }
  {
    {
        Automated processing of real-time pollutant concentration data, using python and GIS tools.
      },
    {
        Development of a tool to assess compliance with air quality international health guidelines.
      }
  }

  %redbull
  \experienceElement
  {Event specialist - “Citycrusher” Team}{Sep. 2017 -- Sep. 2020}
  {Red Bull S.r.l.}{Milan, Italy}
  {Managining brand-activation of \textit{Red Bull Citycrusher} event-car in selected sport
    events in Italy.}
  {}

  %gopro
  \experienceElement
  {Hero Team Promoter}{May. 2014 -- Sep. 2019}
  {GoPro EMEA GmbH}{Munich, Germany}
  {Promotion of cameras and brand-activation in selected events in Europe.}
  {}
\end{sectionElementsList}

% % -----------Multiple Positions Heading-----------
% % \subSubHeading
% %   {Software Engineer I}{Oct 2014 - Sep 2016}
% %   \experienceElementSubListStart
% %     \experienceElementSubListItem{Apache Beam}
% %       {Apache Beam is a unified model for defining both batch and streaming data-parallel processing pipelines}
% %   \experienceElementSubListEnd
% %-------------------------------------------
% \experienceElementListEnd


%-----------EDUCATION-----------
\section{Education}
\begin{sectionElementsList}
  % polimi msc
  \experienceElement
  {BSc \& MSc in Environmental and Land Planning Engineering}{Milan, Italy}
  {Politecnico di Milano}{Sep. 2014 -- Dec. 2020}
  {
    \scriptsize
    \textbf{MSc}: Data structures, analysis \& ML for environment - Advanced Statistics - Advanced GIS - Databases and Spatial SQL
    \newline
    \textit{“Spatial interpolation methods for producing high-resolution PM2.5
      concentration maps in Milan, Italy“}

    \textbf{BSc}:  Soil \& groundwater remediation - Sustainability - Climate change mitigation - Hydrology - Hydraulics - Geology
    \newline
    \textit{“Environmental and economical impact of Case dell'Acqua,a
      free drinking-water distribution project in Milan by MM S.p.A.“}
  }{}

  % bsb
  \experienceElement
  {The British School of Brussels}{Brussels, Belgium}
  {International Baccalaureate (IB Diploma)}{Sep. 2012 -- May. 2014}
  {}{}
\end{sectionElementsList}

%-----------LICENSES-----------
\section{Licenses}
\begin{sectionElementsList}
  \skillsElement{1}{
    \skillsItem{Italian National Assotiation of Engineers}{2021}
  }
\end{sectionElementsList}

% %-----------TECH SKILLS-----------
\section{Technical Skills}
\begin{sectionElementsList}
  \skillsElement{2}{
    \skillsItem{Programming}{Python, JavaScript \& shell}
    \skillsItem{Web frameworks}{Flask, Django \& Express}
    \skillsItem{Database}{ SQL, PostGIS \& noSQL (MongoDB)}
    \skillsItem{Cloud}{AWS, GCP \& Azure}
    \skillsItem{Development}{git, testing, docs \& CI/CD pipelines}
    \skillsItem{API}{HTTP, REST \& GraphQL}
    \skillsItem{Data Science}{pandas, numpy, scikit-learn, tensorflow, matplotlib, seaborn, plotly, geopandas, rasterio}
    \skillsItem{GIS \& CAD}{ArcGIS \& QGis, AutoCAD, Civil3D}
    \skillsItem{Business Intelligence}{Power BI (M \& DAX)}
    \skillsItem{Writing}{\LaTeX\:, Markdown \& reStructuredText}
    \skillsItem{Web}{HTML \& CSS}
  }
\end{sectionElementsList}

%-----------PROJECTS-----------
\section{Projects}
\begin{sectionElementsList}
  \projectElement{
    \href{https://github.com/giocaizzi/py3dinterpolations}{py3dinterpolations}
    }{
    quick 3D spatial interpolations
  }{python, pandas \& geopandas, scikit-learn, numpy, shapely}

  \projectElement{
    \href{https://github.com/giocaizzi/pysurfline}{pysurfline}
    }{
    wrapper for surfline.com API
  }{python, pandas, requests, APIs}

  \projectElement{
    \href{https://github.com/giocaizzi/repomanager}{repomanager}
    }{
    easily manage GitHub repositories
  }{python, Flask, Jinja2, HTML, CSS, APIs}

  \projectElement{
    \href{https://github.com/giocaizzi/smart-spotify}{smart-spotify}
    }{
    create smart playlists with Spotify
  }{JavaScript, node.js, Express, HTML, CSS, APIs}

  \projectElement{
    \href{https://github.com/giocaizzi/mplstrater}{mplstrater}
    }{
      plot stratigraphic logs with matplotlib
    }{python, pandas \& geopandas , matplotlib, shapely, rasterio}
\end{sectionElementsList}


% %-----------PERSONAL SKILLS-----------
\section{Personal skills}
\begin{sectionElementsList}
  \skillsElement{1}{
    \skillsItem{Languages}{Fluency in Italian (mothertongue) and English. French at professional level.}
    \skillsItem{Interests}{skiing, surfing, road-cycling and outdoors. Cartography, infographics and mapping. Tech and AI.}
  }
\end{sectionElementsList}


%-----------FOOTER TEXT-----------
\rfoot{
  \\
  \textbf{References and letters of recommendation available on request.}
}


%-------------------------------------------
\end{document}